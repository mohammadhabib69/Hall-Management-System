...
#include <termios.h>
#include <unistd.h>
#include <fcntl.h> // add this line for fcntl()
#endif
...
void setColor(int color) {} // Dummy for Linux
#define COLOR_RED 0
...
void pauseAndClear()
{
    ...
    // Loop animation until Enter is pressed
    while (true)
    {
        cout << "\rPress any key to continue" << dots[i % 3] << "   ";
        cout.flush();
...
#else
        // For Mac/Linux - check if input is available
        struct timeval tv;
        fd_set fds;
        tv.tv_sec = 0;
